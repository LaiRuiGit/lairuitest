...
if ($_POST) {
	...
	*/
	...
	if (in_array($uinfo["part_id"], array(1,4,9)) || ($username == "admin") || $debug_mode) { 
...
<?php } ?>
	<tr hidden>
		<td class="left">QQ：</td>
		<td class="right"><input name="qq" value="<?php echo $line["qq"]; ?>" class="input" style="width:140px" <?php echo $ce["qq"]; ?>>  <span class="intro">顾客QQ号码</span></td>
	</tr>
	<tr>
	...
	<tr>
		<td class="left" valign="top">装修类型：</td>
		<td class="right">
	...
	<tr>
		...
		<td class="right">
			...
			<span id="area_from_box" style="display: <?php echo $op == "add" ? "none" : ($line["is_local"] == 2 ? "inline" : "none"); ?>">
				面积：<input name="area" id="area" value="<?php echo $line["area"]; ?>" class="input" size="14" <?php echo $ce["is_local"]; ?>>&nbsp;
				←速填常用地区：<select id="quick_area" class="combo" <?php echo $ce["is_local"]; ?> onChange="byid('area').value=this.value;">
					<option value="" style="color:gray">-面积-</option>
					<?php echo list_option($area_list, "_value_", "_value_"); ?>
				</select>
			</span>
			<span class="intro">顾客来源默认为本地(在新增顾客时)</span>
		</td>
	</tr>
...
<table width="100%" class="edit">
	...
	<tr id="yisheng"> <!--  style="display:<?php echo ($line["status"] == 1 ? "inline" : "none"); ?>;" -->
		...
		<td class="right">
			...
			</select>&nbsp;<span class="intro">当顾客已到店时选择</span>
		</td>
	</tr>
</table>
<?php } ?>
